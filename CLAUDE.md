...
- **Oxfmt config** (`.oxfmtrc.json`): single quotes, semicolons, 2-space indent, trailing commas (ES5), 100 char width, LF line endings
...
- Config: `.oxlintrc.json` — Rust-based linter (Oxc), native TS parsing, no separate parser package
- `typescript`, `unicorn`, `oxc`, `react` plugins enabled; `correctness` category set to error
- `react` plugin rules relaxed:
  ...
  - `use-memo: off` — `useMergedRefs` needs a dynamic-length `useCallback` dep array
- `no-unused-vars` off (TypeScript handles this)
- `cloudflare-worker/`, `dist/`, `public/` are excluded from Oxlint via `ignorePatterns`
