...
import { ArrowUpRight, FileUp, Layers, Lock, ShoppingBag } from 'lucide-react';

// Force the logo to crisp monochrome white so it sits cleanly on the black mock,
// consistent with the treatment used for the other venture visual.
...
// Real, verifiable facts about the store — not a synthetic score. Each stat counts
// up when the visual scrolls into view.
const STATS = [
    { key: 'colors', value: 4, suffix: '' },
    { key: 'quote', value: 48, suffix: 'h' },
    { key: 'shipping', value: 60, suffix: '.–' },
    { key: 'swiss', value: 100, suffix: '%' },
] as const;

function StatCounter({ labelKey, value, suffix }: { labelKey: string; value: number; suffix: string }) {
        ...
        <div className="relative w-full rounded-2xl border border-white/10 bg-black overflow-hidden shadow-2xl">
            ...
            <div className="relative">
                ...
                <div className="flex items-center justify-between px-5 py-3.5 border-b border-white/[0.06]">
                    <img
                        src="/images/swiss3design-icon.svg"
                        alt=""
                        aria-hidden="true"
                        loading="lazy"
                        decoding="async"
                ...
                {/* Hero — wordmark + colorway swatches + skeleton copy + CTA */}
                <div className="px-6 pt-8 pb-6 flex flex-col items-center text-center gap-4">
                    <img
                        src="/images/swiss3design-logo.png"
                        alt="Swiss3Design"
                        loading="lazy"
                        decoding="async"
                    ...
                    <div className="flex items-center gap-2">
                        {['bg-white', 'bg-white/70', 'bg-white/40', 'bg-white/20'].map((tone, i) => (
                            <span
                                key={i}
                                className={`w-3 h-3 rounded-full border border-white/30 ${tone}`}
                            />
                        ))}
                        <span className="ml-1 text-2xs uppercase tracking-wider text-white/40">
                            {t('swiss3design.colors_label')}
                        </span>
                    </div>
...
const PILLARS = [
    { key: 'print', icon: Layers },
    { key: 'custom', icon: FileUp },
    { key: 'stack', icon: ShoppingBag },
] as const;

export function Swiss3Design() {
    const { t } = useTranslation();
                                ...
                                >
                                    ...
                                    <div>
                                        <h3 className="font-semibold text-sm text-foreground">
                                            {t(`swiss3design.pillars.${key}.title`)}
                                        </h3>
                                        <p className="text-muted-foreground text-sm leading-relaxed">
                                            {t(`swiss3design.pillars.${key}.desc`)}
                                        </p>
                                    </div>
                                </div>
                            ))}
