        ...
        for (const o of s.obstacles) {
          ...
          if (px2 > ox1 && px1 < ox2 && py2 > oy1 && py1 < oy2) {
            ...
            if (s.score > lastBest) {
              lastBest = s.score;
              try {
                localStorage.setItem(STORAGE_BEST, String(s.score));
              } catch {
                /* noop */
              }
              setBest(s.score);
      ...
      <div className="w-full max-w-3xl">
        ...
        >
          ...
          <div className="absolute top-3 right-4 font-mono text-xs uppercase tracking-widest text-muted-foreground flex gap-4">
            <span>
              HI <span className="text-foreground font-bold">{String(best).padStart(5, '0')}</span>
            </span>
            <span>
              <span className="text-foreground font-bold">{String(score).padStart(5, '0')}</span>
            </span>
          </div>
              ...
              <div className="text-center font-mono">
                {gameOver ? (
                  <>
                    ...
                    <div className="text-xs uppercase tracking-widest text-foreground border border-foreground/30 px-4 py-2 inline-block">
